...
class Enemy:
    ...
    def set_coordinates(self):
        ...
        spawn_area = pygame.Rect(camera_rect.left - s, camera_rect.top - s, camera_rect.width + 2 * s,
                                 camera_rect.height + 2 * s)
        ...
        while True:
            ...
            if not camera_rect.colliderect(enemy_rect) and 0 <= x < GAMESIZE[0] - self.res[0] and 0 <= y < GAMESIZE[1] - \
                    self.res[1]:
                self.pos = v2(x, y)
                break
    ...
    def update_is_attacking(self):
        ...
        elif self.is_attacking and self.frame > len(
                self.game.assets[f"{self.name}_{self.current_animation}_{self.facing}"]):
            self.is_attacking = False
    ...
    def draw(self, surface=None):
        ...
        if self.has_shadow:
            ...
            self.game.displayS.blit(shadow_image, (
            pos[0] + self.res[0] / 2 - shadow_image.width / 2, pos[1] + self.res[1] - shadow_image.height / 2))
