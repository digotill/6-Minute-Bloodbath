...
class Effect:
    ...
    def update(self):
        if not self.grounded and not self.game.changing_settings and not self.game.cards_on:
            ...
            rect = pygame.Rect(self.pos.x - self.res[0] / 6, self.pos.y - self.res[1] / 6, self.res[0] / 3,
                               self.res[1] / 3)
...
class MuzzleFlash:
    def __init__(self, game, pos, rotation, flip, name):
        ...
        self.images = [pygame.transform.flip(pygame.transform.rotate(image, rotation), True,
                                             False) if flip else pygame.transform.rotate(image, -rotation + 180) for
