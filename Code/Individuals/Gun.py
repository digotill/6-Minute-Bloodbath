...
class Gun:
    ...
    def draw(self, surface):
        if not self.game.died and not self.game.won:  # Only draw if player is alive
            ...
            self.rect = self.rotated_image.get_rect(
                center=(pos_x, pos_y + self.game.player.air_offset))  # Update gun rectangle
    ...
    def shoot(self, current_time):
        ...
        self.game.soundM.play_sound(self.name + "_shot", VOLUMES["gun_shot_frequancy"],
                                    VOLUMES["gun_shot_volume"] * self.game.master_volume)  # Play gun shot sound

        self.game.muzzleflashM.add_muzzle_flash(self.calculate_position(2), 270 - self.angle,
                                                True if self.game.player.facing == "left" else False)  # Create muzzle flash
        ...
        for _ in range(self.shots):  # For each shot
            self.game.bulletM.add_bullet(self.calculate_position(3), self.game.methods.change(self.angle, self.spread),
    ...
    def calculate_position(self, ratio):
        start_x = self.game.player.rect.centerx + math.sin(math.radians(self.angle)) * int(
            self.distance - self.res[0] / ratio)  # Calculate bullet start x
        start_y = self.game.player.rect.centery + math.cos(math.radians(self.angle)) * int(
            self.distance - self.res[0] / ratio)  # Calculate bullet start y
