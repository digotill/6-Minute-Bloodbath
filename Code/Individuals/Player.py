...
class Player:
    ...
    def jump(self):
        if not self.is_on_air and self.stamina - self.jump_stamina > 0 and self.jumping_timer.update(
                self.game.game_time):
            self.jump_velocity = self.jumping_velocity
            self.is_on_air = True
            self.stamina -= self.jump_stamina
            self.air_offset = -1
            self.game.soundM.play_sound("jump", VOLUMES["jump_frequancy"],
                                        VOLUMES["jump_volume"] * self.game.master_volume)
    ...
    def update_animation(self):
        ...
        new_animation = 'running' if (self.game.inputM.get("move_left") or self.game.inputM.get("move_right") or
                                      self.game.inputM.get("move_down") or self.game.inputM.get(
                    "move_up")) and not self.game.changing_settings else 'idle'
    ...
    def manage_xp(self):
        if self.xp >= self.max_xp:
            ...
            self.game.soundM.play_sound("level_up", VOLUMES["level_up_frequancy"],
                                        VOLUMES["level_up_volume"] * self.game.master_volume)
        else:
            self.calculate_max_xp()
    ...
    def draw(self, surface=None):
        ...
        surface.blit(shadow_image,
                     (self.get_position()[0], self.get_position()[1] + self.res[1] - shadow_image.height / 2))
    ...
    def handle_stamina(self):
        self.is_sprinting = self.game.inputM.get(
            "sprint") and self.velocity.length() > 0 and not self.game.changing_settings
    ...
    def deal_damage(self, damage):
        ...
        if self.game.game_time - self.last_hit > self.hit_cooldown and not self.is_on_air:
            ...
            if not self.dead:
                ...
                self.game.soundM.play_sound("heartbeat", VOLUMES["heartbeat_frequancy"],
                                            VOLUMES["heartbeat_volume"] * self.game.master_volume)
                integer = str(random.randint(0, 4))
                self.game.soundM.play_sound("splatter" + integer, VOLUMES["splatter_frequancy"],
                                            VOLUMES["splatter_volume"] * self.game.master_volume)
                self.hit_count = 0
    ...
    def handle_slowdown(self):
        collision = self.game.tilemapM.tile_collision(pygame.Rect(self.pos.x, self.pos.y + self.res[1] / 2, 0, 0),
                                                      "water_tile")
        if collision:
