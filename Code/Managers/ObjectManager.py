...
class ObjectManager:
    def __init__(self, game):
        self.game = game
        self.grid = HashMap(game, GENERAL["hash_maps"][
            4])  # Spatial hash grid for efficient object lookup and collision detection
    ...
    def draw_shadows(self):
        for object1 in self.grid.items:
            object1.draw_shadow(
                self.game.tilemapM.cached_surface)  # Draw shadow for each object on the pre-rendered surface
    ...
    def _generate_trees(self):
        ...
        for y in range(0, GAMESIZE[1], size):
            for x in range(0, GAMESIZE[0], size):
                ...
                biome, biome_density_factor = self._get_biome_info(biome_value,
                                                                   sorted_biomes)  # Determine biome and its density factor

                if self._should_place_tree(density_value,
                                           biome_density_factor):  # Check if a tree should be placed here
    ...
    def _is_valid_position(self, x, y, size):
        if 0 <= x < GAMESIZE[0] - size[0] and 0 <= y < GAMESIZE[1] - size[1]:  # Check if position is within game bounds
            rect = pygame.Rect(x - size[0] * 0.25, y + size[1] * 0.5, size[0] / 2,
                               size[1] / 10)  # Create a rect for collision check
    ...
    def _should_place_tree(density_value, biome_density_factor):
        return random.random() < density_value * GENERAL["enviroment_density"][
            0] * biome_density_factor  # Probability check for tree placement
    ...
    def _is_valid_biome_position(self, biome_x, biome_y):
        return 0 <= biome_x < len(self.biome_map[0]) and 0 <= biome_y < len(
            self.biome_map)  # Check if position is within biome map bounds
