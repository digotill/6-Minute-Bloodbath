...
class ScreenEffectManager:
    ...
    def initialize_screen_effects(self):
        self.transition_effect = ScreenEffect(self.game, self.game.assets["transition_screeneffect"],
                                              GENERAL['animation_speeds'][1])
        self.youdied_effect = ScreenEffect(self.game, self.game.assets["youdied_screeneffect"],
                                           GENERAL['animation_speeds'][2])
        self.blood_effect = ScreenEffect(self.game, self.game.assets["blood_screeneffect"],
                                         GENERAL['animation_speeds'][6])
        self.youwon_effect = ScreenEffect(self.game, self.game.assets["youwin_screeneffect"],
                                          GENERAL['animation_speeds'][2])
        ...
        if self.game.won:
            if self.youwon_start_time is None:
                ...
                if self.game.difficulty == "easy":
                    ...
                    self.game.wins += 3
                self.game.soundM.play_sound("youwon_sound", VOLUMES["youwon_frequancy"],
                                            VOLUMES["youwon_volume"] * self.game.master_volume)
    ...
    def handle_you_died_effect(self):
        if self.game.died:
            if self.youdied_start_time is None:
                ...
                pygame.mixer.music.stop()
                self.game.soundM.play_sound("youdied_sound", VOLUMES["youdied_frequancy"],
                                            VOLUMES["youdied_volume"] * self.game.master_volume)
