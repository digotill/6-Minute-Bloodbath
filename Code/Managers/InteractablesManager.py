...
class InteractablesManager:
    ...
    def create_buttons(self):
        ...
        for name, config in button_configs.items():
            ...
            button_class = Switch if name in ['easy', 'medium', 'hard'] or name in BUTTONS[
                "Weapon_Buttons"].keys() else Button
    ...
    def _update_ingame_buttons(self):
        ...
        if self.game.inputM.get("left_click") and self.button_cooldown_timer.check(
                self.game.ticks) and not self.grabbing_slider and self.game.changing_settings:
    ...
    def _update_menu_buttons(self):
        ...
        if self.game.inputM.get("left_click") and self.button_cooldown_timer.check(
                self.game.ticks) and not self.grabbing_slider:
    ...
    def play_click(self):
        self.game.soundM.play_sound("click", VOLUMES["click_shot_frequancy"],
                                    VOLUMES["click_shot_volume"] * self.game.master_volume)
    ...
    def draw(self):
        ...
        if not self.game.in_menu and not self.game.died and not self.game.won:
            ...
            for button in sorted(
                    list(self.game_buttons.values()) + list(self.sliders.values()) + list(self.bars.values()),
                    key=lambda element: element.pos.y):
                button.draw()
