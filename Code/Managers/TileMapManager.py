...
class Tile:
    def __init__(self, game, tile_type, position):
        ...
        self.rect = pygame.Rect(self.position.x, self.position.y, self.size - 1,
                                self.size - 1)  # Rectangular area of the tile
...
class TileMapManager:
    ...
    def _generate_maps(self):
        ...
        density_noise = PerlinNoise(octaves=MAP["biomes_density_map"][1],
                                    seed=random.randint(0, 100000))  # Create density noise
        return (
    ...
    def _generate_noise_map(noise, scale):
        size = GENERAL["enviroment_density"][1]  # Get environment density size
    ...
    def add_tile(self, tile_type, grid_position):
        pixel_position = (
        grid_position[0] * self.tile_size, grid_position[1] * self.tile_size)  # Convert grid to pixel position
    ...
    def apply_transition_tiles(self, transition_array, count=0):
        ...
        for tile in self.grid.items:
            if tile.tile_type == transition_array[1]:
                ...
                neighbours = [self.get((grid_x + dx, grid_y + dy)) for dx, dy in
                              [(0, -1), (0, 1), (1, 0), (-1, 0)]]  # Get neighboring tiles (top, bottom, right, left)
                ...
                neighbours_string = ''.join(
                    map(neighbor_value, neighbours))  # Create string representation of neighbors
                ...
                elif self.count_surrounding_tiles(tile) == 4 and count == 0 and string in ["11100100", "00111001",
                                                                                           "01001110", "10010011",
                                                                                           "10100100", "00101001",
                                                                                           "01001010", "10010010"]:
                    changes = change_tile(changes)
                elif self.count_surrounding_tiles(tile) == 4 and count == 1 and (
                        "101" in string or string in ["1100010"]):
                    changes = change_tile(changes)
                ...
                elif neighbours_string in ["2121", "2112", "1221", "1212", "1222", "2122", "2212", "2221",
                                           "2222"] and count == 3 and corner_string is not True:
                    self.add_grid2_tile(tile, grid_x, grid_y, transition_array, corner_string)
                elif self.count_surrounding_tiles(tile) == 3 and count == 4 and string in ["01011011", "01101101",
                                                                                           "10110101", "11010110"]:
                    changes = change_tile(changes)
