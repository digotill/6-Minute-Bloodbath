...
class EnemyManager:
    ...
    def _add_enemies(self):
        ...
        for key in BOSSES.keys():
            if self.game.game_time >= key and not getattr(self, BOSSES[key] + "_spawned", False) and GENERAL["enemies"][
                2]:
    ...
    def remove_dead_enemies(self):
        for enemy in self.grid.items.copy():
            ...
            if enemy.dead:
                ...
                self.game.experienceM.add_experience(xp_type,
                                                     enemy.rect.center)  # Add enemy's experience to player's total
